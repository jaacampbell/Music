...
def _phase14_package(job_dir: Path) -> Path:
    package = _original_package(job_dir)
    _persist_outputs(job_dir.name, job_dir)
    return package


def _phase14_cloud_row(job: dict[str, Any]) -> dict[str, Any] | None:
...
base._package = _phase14_package
phase5._cloud_row = _phase14_cloud_row


@app.get("/agent/persistence/cloud")
def phase14_persistence_health() -> dict[str, Any]:
    return {
        "status": "ok",
...
def phase14_result_state(request: Request, job_id: str) -> dict[str, Any]:
